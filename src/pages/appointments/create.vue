...
export default {
  ...
  created() {
    this.editTime = moment(this.selectedDate).format('LT')
  },
  methods: {
    ...
    saveAppointment() {
      ...
      this.form.date = `${moment(this.selectedDate).format('MM/DD/YYYY')} ${moment(this.selectedDate).format('LTS')}`
      EventBus.$emit('AppointmentCreate')
    }
  }
}
</script>
