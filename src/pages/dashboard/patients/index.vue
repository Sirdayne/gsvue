...
<div class="dashboard-container">
  ...
  <div class="alpino-table" v-loading="loading">
    <div class="alpino-card" v-for="(user, index) in getFilteredData" :key="index">
      ...
      <social></social>
      <p>{{ user.email }}</p>
      <el-button size="mini" icon="el-icon-zoom-in" style="margin-top: 15px;" @click="viewDetail(user)"></el-button>
    </div>
  </div>
  <div v-if="false">
    <b-table
    ...
    >
      <template slot-scope="props">
        ...
        <b-table-column label="Details" width="100" centered>
          <el-button class="operation-button" size="mini" type="primary" icon="el-icon-zoom-in" circle @click="viewDetail(props.row)"></el-button>
        </b-table-column>
      </template>
    </b-table>
  </div>
...
export default {
  ...
  methods: {
    viewDetail(row) {
      this.$router.push(`/dashboard/patients/${row.id}`)
    },
    fetchData() {
      this.loading = true
